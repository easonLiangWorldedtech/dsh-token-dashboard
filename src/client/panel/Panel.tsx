// The Token dashboard panel (05 Variant A, production rewrite):
// header controls (tz toggle / refresh / close), stats row, week/day tabs,
// heatmap with 26-week paging, day view, footer notes.
//
// Data policy (05): load on open + manual refresh; no polling, no SSE.
...
import { fmt } from '../fmt'
import { panelStore, closePanel, setTz, tzStore } from '../store'
...
export function TokenPanel({ t }: PanelProps) {
  ...
  const tz = useSyncExternalStore(tzStore.subscribe, tzStore.getSnapshot)
  ...
  return (
    <div className="td-panel" role="dialog" aria-label={t('title')}>
      <div className="td-head">
        <h2>{t('title')}</h2>
        <label className="td-seg" aria-label={t('tzLabel')}>
          <button className={tz === 'local' ? 'on' : ''} onClick={() => setTz('local')}>{t('tzLocal')}</button>
          <button className={tz === 'utc' ? 'on' : ''} onClick={() => setTz('utc')}>{t('tzUtc')}</button>
        </label>
        <span className="td-spacer" />
        <button className="td-btn" onClick={() => void load()}>{t('refresh')}</button>
